<template>
  ...
  <div
    v-if="properties.length && instance.values.hasOwnProperty('additional_streams')"
    class="space-y-1 px-4 sm:space-y-0 sm:grid sm:grid-cols-1 sm:gap-4 sm:px-6 sm:py-5"
  >
    <PropertyDisclosure :title="`Stream ${streamIndex+1} - ${stream.data.name} - ${stream.data.fluid}`"
                        v-for="(stream, streamIndex) in instance.values.additional_streams"
    >
      <div
        class="my-6"
        v-for="(property, propertyIdx) in getStreamProperties(stream)"
        :key="propertyIdx"
      >
        <TextInput
  ...
  setup(props) {
    ...
    const getStreamProperties = (stream) => {
        ...
        return properties.map((p) => {
            ...
            if (stream.data[p.property.symbolic_name] == null)
                return {
                    label: label,
                    description: description,
                    value: "Not Defined",
            ...
            if (p.property.inputType === "select")
                return {
                    ...
                    value: p.property.data.options.find(
                        (o) =>
                            o.key ===
                            stream.data[p.property.symbolic_name]
                    ).value,
                };
            ...
            return {
                label: label,
                description: description,
                value: stream.data[p.property.symbolic_name],
            };
        });
    }
