  ...
  <app-layout>
    ...
    >
      <div class="w-full h-96 md:h-full md:w-1/2">
        ...
        >
          <template #header> Sources </template>
          <template #description> A list of all the Sources </template>
        </index-table>
      </div>
    ...
    >
      <template #header> Delete Source </template>
  ...
  setup(props) {
    ...
    function onItemRemoved(item) {
      ...
      const marker = markers.value.find(
        (element) => element.id === item.location.value.geo_object.id
      );
    ...
    function populateAllMarkers() {
      markers.value = useUniqueLocations(props.sources).value.map(
    ...
    async function onDelete() {
      await Inertia.delete(
        route("objects.sources.destroy", entryToDelete.value.id)
      );
