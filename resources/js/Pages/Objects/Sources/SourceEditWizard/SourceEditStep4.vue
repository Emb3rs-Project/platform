...
export default {
    ...
    setup(props, ctx) {
        ...
        const storeAdditionalStreams = computed(() => props.streams);
        ...
        for (const p of properties.value) {
            for(let index in additionalStreams.value) {
                const value = additionalStreams.value[index].data[p.property.symbolic_name]

                if (typeof value != 'undefined') {
                    if (p.property.inputType === "select") {
                        additionalStreams.value[index].data[p.property.symbolic_name] = p.property.data.options.find(
                            (o) => o.key === value
                        );
                    }
                }
